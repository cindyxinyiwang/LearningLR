...
%First, for low-resource MT, the use of RNNs was a conscious decision: we have found that RNN-based models are significantly less sensitive to hyperparameters than Transformers in very low-resource settings. Note that we are not the only ones: a recent ACL 2019 paper addressing low-resource NMT
% ~\citep{lownmt19} 
...
\textbf{Uniform} where data is selected uniformly from all of the data that we have available, as is standard in training models. \textbf{SPCL}~\citep{}, a curriculum learning method that focuses more on the ``easy'' training examples based on model loss, so that the training curriculum is dynamically updated throughout training.
\textbf{\dds}, our proposed method.

For image classification, we have two additional methods for the Cifar-10 experiments. \textbf{MentorNet}~\citep{}: a recently proposed curriculum learning method that optimizes a neural network to determine the training curriculum of the main model. \textbf{retrained \dds}: we first train a model and a scorer using the standard \dds till convergence; then we reinitialize the model parameters and train the model again using \dds.
